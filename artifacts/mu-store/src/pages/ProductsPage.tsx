...
import { SlidersHorizontal, Grid, List, X, Heart, ShoppingBag } from "lucide-react";
    ...
    <motion.div whileHover={{ y: -4 }} className="group" data-testid={`card-product-${p.id}`}
      ...
      <Link href={`/products/${p.id}`}>
        <div className="aspect-[3/4] rounded-xl overflow-hidden bg-muted relative">
          {currentImg && (
            <img src={currentImg} alt={p.name} className="w-full h-full object-cover transition-all duration-500" loading="lazy" />
          )}
          <div className="absolute top-2 left-2 flex flex-col gap-1">
            {p.isNew && <span className="bg-foreground text-background text-xs px-2 py-0.5 rounded font-medium">New</span>}
            {p.isSale && <span className="bg-[#D4608A] text-white text-xs px-2 py-0.5 rounded font-medium">Sale</span>}
            {p.stock <= 3 && p.stock > 0 && <span className="bg-amber-500 text-white text-xs px-2 py-0.5 rounded font-medium">Only {p.stock} left</span>}
          </div>
          <div className="absolute inset-x-0 bottom-0 p-3 opacity-0 group-hover:opacity-100 transition-opacity">
            <Button onClick={handleQuickAdd} size="sm" className="w-full bg-background text-foreground hover:bg-background/90 text-xs font-medium" data-testid={`button-quick-add-${p.id}`}>
              <ShoppingBag size={12} className="mr-1.5" /> Quick Add
            </Button>
          </div>
        </div>
...
export default function ProductsPage() {
  ...
  const FiltersPanel = () => (
    <div className="space-y-6">
        ...
        <div className="space-y-2">
          {[{ slug: "", name: "All" }, ...(categories ?? [])].map(cat => (
            <button key={cat.slug} onClick={() => { setCategory(cat.slug); setPage(1); }}
              ...
              {cat.name} {"productCount" in cat ? `(${cat.productCount})` : ""}
            </button>
          ))}
        </div>
      </div>
      ...
      {(category || selectedSize || selectedColor) && (
        <Button variant="outline" size="sm" onClick={() => { setCategory(""); setSelectedSize(""); setSelectedColor(""); setPriceRange([0, 3000]); setPage(1); }} className="w-full">
        ...
        <div className="flex-1 min-w-0">
          {isLoading ? (
            ...
            <div className="text-center py-24">
              ...
              <Button variant="outline" onClick={() => { setCategory(""); setSelectedSize(""); setSelectedColor(""); setPriceRange([0, 3000]); }}>Clear All Filters</Button>
            </div>
          ) : (
            <>
