...
function HeroSection() {
  ...
  return (
    <section className="relative min-h-[85vh] flex items-center overflow-hidden bg-foreground">
      <div className="absolute inset-0 opacity-10" style={{ backgroundImage: "radial-gradient(circle at 30% 50%, #C9A96E 0%, transparent 60%), radial-gradient(circle at 80% 20%, #D4608A 0%, transparent 50%)" }} />
        ...
        <div className="max-w-2xl">
          ...
          <div className="h-32 sm:h-24 overflow-hidden">
            <motion.h1 key={idx} initial={{ y: 40, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -40, opacity: 0 }} transition={{ duration: 0.5 }}
              className="font-serif text-5xl sm:text-6xl lg:text-7xl font-bold text-background leading-tight">{headlines[idx]}</motion.h1>
          </div>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="mt-6 text-background/70 text-lg max-w-md leading-relaxed">
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="flex items-end justify-between mb-8">
        <div><p className="text-xs tracking-widest uppercase text-[#C9A96E] mb-1">Fresh Arrivals</p><h2 className="font-serif text-3xl sm:text-4xl font-bold">New Arrivals</h2></div>
        <Link href="/products?sort=newest" className="text-sm font-medium flex items-center gap-1 text-muted-foreground hover:text-foreground transition-colors">View All <ArrowRight size={14} /></Link>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-4 gap-y-6">
        {isLoading ? Array(6).fill(0).map((_, i) => <Skeleton key={i} className="aspect-[3/4] rounded-xl" />)
            ...
            <Link href={`/products/${p.id}`}>
              ...
              <div className="mt-3">
                ...
                <div className="flex items-center gap-2 mt-1">
                  {p.salePrice ? (<><span className="font-bold text-[#D4608A]">{p.salePrice.toLocaleString()} EGP</span><span className="text-xs text-muted-foreground line-through">{p.price.toLocaleString()}</span></>) : <span className="font-semibold text-sm">{p.price.toLocaleString()} EGP</span>}
                </div>
              </div>
            </Link>
          </motion.div>
  ...
  return (
    <section className="bg-muted/30 py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-10"><p className="text-xs tracking-widest uppercase text-[#C9A96E] mb-1">Explore</p><h2 className="font-serif text-3xl sm:text-4xl font-bold">Shop by Category</h2></div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {TRUST_BADGES.map(({ icon: Icon, label, sub }) => (
          <div key={label} className="flex flex-col items-center text-center gap-3 p-6 rounded-2xl border border-border hover:border-[#C9A96E] transition-colors">
            <div className="w-12 h-12 rounded-full bg-[#C9A96E]/10 flex items-center justify-center"><Icon size={22} className="text-[#C9A96E]" /></div>
            <div><p className="font-semibold text-sm">{label}</p><p className="text-xs text-muted-foreground mt-0.5">{sub}</p></div>
          </div>
        ))}
      </div>
    </section>
...
function BestSellersSection() {
  ...
  return (
    <section className="bg-foreground py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between mb-8">
          <div><p className="text-xs tracking-widest uppercase text-[#C9A96E] mb-1">Customer Favorites</p><h2 className="font-serif text-3xl sm:text-4xl font-bold text-background">Best Sellers</h2></div>
          <Link href="/products?sort=best_selling" className="text-sm font-medium flex items-center gap-1 text-background/60 hover:text-background transition-colors">View All <ArrowRight size={14} /></Link>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {isLoading ? Array(4).fill(0).map((_, i) => <Skeleton key={i} className="aspect-[3/4] rounded-xl opacity-20" />)
          : products.map(p => (
            <Link key={p.id} href={`/products/${p.id}`} className="group" data-testid={`card-bestseller-${p.id}`}>
              ...
              <p className="font-medium text-sm text-background mt-3 leading-tight">{p.name}</p>
