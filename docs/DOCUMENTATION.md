...
Most server communication uses auto-generated TanStack Query hooks from `lib/api-client-react/src/generated/`. These are generated by Orval from the OpenAPI spec. Examples: `useListProducts()`, `useLogin()`, `useGetCart()`, `useCreateOrder()`. Some features that are **not** in the OpenAPI spec — including the virtual try-on flow (`/api/tryon/*`), settings reads, and parts of the admin panel — use direct `fetch()` or `axios` calls instead of generated hooks.
...
Core CRUD routes (products, orders, auth, cart, wishlist, reviews) validate input with Zod schemas generated from the OpenAPI spec via `@workspace/api-zod`. Invalid input returns HTTP 400 with `{ error: "Invalid input" }`. Several other routes — including brands, testimonials, settings, admins, and contact — use ad-hoc field checks rather than generated Zod schemas; they may return different error shapes.
...
> **Source of truth precedence:** The OpenAPI spec (`lib/api-spec/openapi.yaml`) covers only the core routes that are codegen-backed. The live route files under `artifacts/api-server/src/routes/` are the canonical runtime source of truth. Where they differ from the spec (e.g. the `top_rated` sort value vs. `most_reviewed` in the spec, or routes absent from the spec entirely), the runtime behaviour documented here reflects the actual route files.
...
| `sort` | enum | `price_asc` | `newest` / `price_asc` / `price_desc` / `best_selling` / `top_rated` (runtime); OpenAPI spec lists `most_reviewed` instead of `top_rated` — runtime behaviour is authoritative |
